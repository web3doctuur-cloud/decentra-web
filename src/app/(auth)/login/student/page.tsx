...
import { BookOpen, AtSign, Lock, ArrowLeft } from "lucide-react";
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-50 dark:bg-black px-4 font-sans">
      <div className="w-full max-w-md p-8 space-y-8 bg-white dark:bg-zinc-900 rounded-3xl shadow-xl border border-zinc-200 dark:border-zinc-800">
        <div className="text-center space-y-2">
          ...
          <h2 className="text-3xl font-bold text-black dark:text-white leading-tight">Student Login</h2>
          <p className="text-zinc-600 dark:text-zinc-400">Welcome back, learner.</p>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
          <Link href="/login" className="text-sm text-zinc-500 hover:text-blue-600 flex items-center gap-1 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Change type
          </Link>
          <Link href="/signup/student" className="text-sm font-bold text-blue-600 hover:underline">
            Create account
          </Link>
        </div>
      </div>
